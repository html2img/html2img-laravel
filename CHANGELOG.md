...
- Documentation for PDF output: pass `format: Format::Pdf` on `HtmlRequest` or
  `ScreenshotRequest` (from `html2img/html2img-php` 1.1) to receive an A4
  portrait vector PDF, and store it with the existing `store()` helper.

### Changed

- Require `html2img/html2img-php` `^1.1`, which restores the `format` option.
...
[1.1.0]: https://github.com/html2img/html2img-laravel/releases/tag/v1.1.0
